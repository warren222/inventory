    ...
    Public Sub updatestock(ByVal stockno As String, ByVal reference As String, ByVal jo As String)
        Try
            ...
            Dim str As String = "
                                    ...
                                    declare @cancelalloc as decimal(10,2)=(selecisnull(sum(isnull(qty,0)),0)from trans_tb where stockno ='" & stockno & "' AND TRANSTYPE='CancelAlloc')+0
                                    ...
                                    declare @productionallocation as decimal(10,2)=(select isnull(sum(isnull(qty,0)),0) from trans_tb where stockno ='" & stockno & "' AND TRANSTYPE='Allocation' and not PRODUCTIONALLOCATION = '')+0
...
", "")
    ...
    Public Sub autoupdatestock2(ByVal stockno As String, ByVal md As String)
        Try
            ...
            Dim str As String = "
                                   declare @return as decimal(10,2)=(select isnull(sum(isnull(qty,0)),0) from trans_tb where stockno ='" & stockno & "' AND TRANSTYPE='Return'  and transdate <= '" & md & "')+0
                                    declare @addadjustment as decimal(10,2)=(select  isnull(sum(isnull(qty,0)),0) from trans_tb where stockno ='" & stockno & "' AND TRANSTYPE='+Adjustment'  and transdate <= '" & md & "')+0
                                    declare @minadjustment as decimal(10,2)=(select  isnull(sum(isnull(qty,0)),0) from trans_tb where stockno ='" & stockno & "' AND TRANSTYPE='-Adjustment'  and transdate <= '" & md & "')+0
                                    declare @receipt as decimal(10,2)=(select isnull(sum(isnull(qty,0)),0) from trans_tb where stockno ='" & stockno & "' AND TRANSTYPE='Receipt' AND NOT XYZ='Order'  and transdate <= '" & md & "')+0
                                    declare @issue as decimal(10,2)=(select  isnull(sum(isnull(qty,0)),0) from trans_tb where stockno ='" & stockno & "' AND TRANSTYPE='Issue' AND NOT XYZ ='Allocation'  and transdate <= '" & md & "')+0
                                    declare @receiptorder as decimal(10,2)=(select  isnull(sum(isnull(qty,0)),0) from trans_tb where stockno ='" & stockno & "' AND TRANSTYPE='Receipt' AND XYZ='Order'  and transdate <= '" & md & "')+0
                                    declare @issueallocation as decimal(10,2)=(select  isnull(sum(isnull(qty,0)),0) from trans_tb where stockno ='" & stockno & "' AND TRANSTYPE='Issue' AND XYZ ='Allocation'  and transdate <= '" & md & "')+0
                                    declare @totalreceipt as decimal(10,2)=@receipt+@receiptorder
                                    declare @totalissue as decimal(10,2)=@issue+@issueallocation
            update stocks_tb set 
